...
const createServer = () => {
    ...
    app.get('/hello', (req, res) => {
        res.status(200).send('hello this is Nedal')
    });
    app.get('/aboutme', (rq, res) => {
        res.status(200).send('Electrical Engineer')
    });
    app.get('*', (req, res) => {
...
const count = (target, input) => {
    // Solution code here...
    let count = input.reduce((accu, ele, i) => {
        let innerArr = ele.reduce((innerAccu, innerEle) => {
            if (innerEle === target) {
                innerAccu++;
            };
            return innerAccu;
...
const totalSum = (input) => {
    // Solution code here...
    let totalSum = input.reduce((accum, ele) => {
        let innerSum = ele.reduce((innerAcc, inEle) => {
            return innerAcc += inEle
        }, 0);
...
const divisibleByFiveTwoToThePower = (input) => {
    // Solution code here...
    let creat = input.map(ele => {
        let check = ele.filter(ele => {
            return !(ele % 5);
        });
        return check;
    });

    let squer = creat.map(ele => {
        let inner = ele.map(innerEle => {
            return Math.pow(2, innerEle);
        });
        return inner;
    });
    return squer;
